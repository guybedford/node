...
Serializes a compiled [`WebAssembly.Module`][] into a buffer containing its
wire bytes together with the machine code that V8 has generated for it so far,
for use with [`deserializeWasmModule()`][].

Only code compiled by V8's optimizing tier is included. By default V8 compiles
WebAssembly functions lazily with a baseline compiler and only optimizes
functions that run hot, so a module that is serialized right after compilation
typically contains no machine code, while a module serialized after a warm-up
period contains the optimized code for the functions that were executed. To
compile all functions with the optimizing tier at module creation, so that the
serialized form is complete right away, start Node.js with:

```console
$ node --no-liftoff --no-wasm-lazy-compilation app.js
```

The machine code is only valid for the exact Node.js binary and V8 flags that
produced it. It is safe to store to disk, since incompatible code is detected
and recompiled by [`deserializeWasmModule()`][].

### `v8.deserializeWasmModule(buffer[, options])`
...
* `buffer` {Buffer|TypedArray|DataView} A buffer returned by
  [`serializeWasmModule()`][].
* `options` {Object} The compile options the module was originally compiled
  with, as accepted by the [`WebAssembly.Module`][] constructor.
  * `builtins` {string\[]}
  * `importedStringConstants` {string}
* Returns: {WebAssembly.Module}

Creates a [`WebAssembly.Module`][] from a buffer returned by
[`serializeWasmModule()`][]. If the serialized machine code is compatible with
the current process it is used directly, otherwise the module is compiled
from the wire bytes as if by `new WebAssembly.Module(wireBytes, options)`.

`options` must match the options used to compile the original module,
otherwise the serialized machine code is rejected and the module is compiled
from the wire bytes with the given options instead.

```mjs
import { serializeWasmModule, deserializeWasmModule } from 'node:v8';
import { readFile, writeFile } from 'node:fs/promises';

const wireBytes = await readFile('module.wasm');
const module = new WebAssembly.Module(wireBytes);
// ... run the module so that hot functions are optimized ...
await writeFile('module.wasm.cache', serializeWasmModule(module));

// In a later process:
const cached = deserializeWasmModule(await readFile('module.wasm.cache'));
```
...
[`deserializeWasmModule()`]: #v8deserializewasmmodulebuffer-options
...
[`serializeWasmModule()`]: #v8serializewasmmodulemodule
