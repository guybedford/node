...
(function bootstrapNodeJSCore(process,
  ...
  function startup() {
    ...
    if (internalBinding('worker').getEnvMessagePort() !== undefined) {
      ...
      if (process._eval != null && !process._forceRepl) {
        ...
        if (process.binding('config').module)
          evalModule();
        else
          evalScript('[eval]');
  ...
  function evalModule() {
    ...
    const { loaderPromise } =
      NativeModule.require('internal/process/esm_loader');
    const { getURLFromFilePath } = NativeModule.require('internal/url');
    const path = NativeModule.require('path');
    loaderPromise.then(async (loader) => {
      const { result } =
        await loader.eval(process._eval,
                          getURLFromFilePath(path.resolve('[eval]')).href);
      if (process._print_eval) console.log(result);
    })
    .catch((e) => {
  ...
  function checkSyntax(source, filename) {
    let isModule = false;
    if (process.binding('config').module || filename.endsWith('.mjs')) {
      if (filename === '[stdin]' || filename === '[eval]') {
        ...
        if (ext === '.mjs' || ext === '.js') {
          ...
          } = NativeModule.require('internal/errors').codes;
