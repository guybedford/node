  ...
  function startup() {
    ...
    if (NativeModule.exists('_third_party_main')) {
      ...
      if (process._eval != null && !process._forceRepl) {
        ...
        if (process.binding('config').mainMode === 'esm')
          evalModule();
        else
          evalScript('[eval]');
        ...
        if (process._forceRepl || NativeModule.require('tty').isatty(0)) {
          if (process.binding('config').mainMode) {
            const err = new Error('--mode flag not supported in REPL');
  ...
  function checkSyntax(source, filename) {
    ...
    if (process.binding('config').mainMode === 'esm' ||
      ...
      if (filename === '[stdin]' || filename === '[eval]') {
        ...
        if (ext === '.mjs' || ext === '.js') {
          isModule = true;
        } else if (ext !== '.json' && ext !== '.node') {
          const err = new Error(`Unknown file extension for ${filename}`);
          err.code = 'ERR_UNKNOWN_FILE_EXTENSION';
          err.name = 'Error [ERR_UNKNOWN_FILE_EXTENSION]';
          throw err;
        }
      }
    }
