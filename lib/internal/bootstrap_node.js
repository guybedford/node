...
(function(process) {
  ...
  function startup() {
    ...
    if (NativeModule.exists('_third_party_main')) {
      ...
      if (process._eval != null && !process._forceRepl) {
        ...
        if (process.binding('config').mainFormat === 'esm')
          evalModule();
        else
          evalScript('[eval]');
        ...
        if (process._forceRepl || NativeModule.require('tty').isatty(0)) {
          if (process.binding('config').mainFormat) {
            const err = new Error('--format flag not supported in REPL');
            err.code = 'ERR_INVALID_REPL_EVAL_FLAG';
            err.name = 'Error [ERR_INVALID_REPL_EVAL_FLAG]';
            throw err;
          }
          // REPL
  ...
  function evalModule() {
    const { decorateErrorStack } = NativeModule.require('internal/util');
    const { loaderPromise } = NativeModule.require('internal/module');
    loaderPromise.then(async (loader) => {
      const { result } = await loader.eval(process._eval);
      if (process._print_eval) console.log(result);
    })
    .catch((e) => {
  ...
  function checkScriptSyntax(source, filename) {
    let format = process.binding('config').mainFormat;
    if (!format)
      format = filename.endsWith('.mjs') ? 'esm' : 'commonjs';
    switch (format) {
      case 'esm':
      ...
      case 'json':
        ...
        break;
      default:
        const err = new Error(`Unable to check module format: ${format}`);
        err.code = 'ERR_INVALID_MODULE_FORMAT';
        err.name = 'Error [ERR_INVALID_MODULE_FORMAT]';
        throw err;
    }
  }
