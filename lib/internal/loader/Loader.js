...
class Loader {
  constructor() {
    ...
    // index for assigning unique URLs to anonymous module evaluation
    this.evalIndex = 0;
  }
  ...
  async eval(source, url = `eval:${++this.evalIndex}`) {
    ...
    const job = new ModuleJob(this, url, evalInstance, inspectBrk);
  ...
  static registerImportDynamicallyCallback(loaderPromise) {
