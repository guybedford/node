...
class Loader {
  constructor(resolver = resolve, base = getBase()) {
  ...
  async resolve(specifier, parentURLOrString = this.base) {
    ...
    let { url, loader } = await this.resolver(specifier, parentURLOrString, resolve);
    if (typeof loader !== 'string') {
      if (typeof loader !== 'function')
        throw new errors.TypeError('ERR_INVALID_LOADER', loader);
      return { url, loader };
    }
  ...
  async getModuleJob(specifier, parentURLOrString = this.base) {
    ...
    if (job === undefined) {
      let loaderInstance;
      if (typeof loader === 'string') {
        loaderInstance = loaders.get(loader);
      }
      else {
