...
class Loader {
  constructor({ resolve = ModuleRequest.resolve, dynamicInstantiate } = {}, base = getBase()) {
  ...
  async resolve(specifier, parentURLOrString = this.base) {
    ...
    if (typeof resolved.format !== 'string') {
      throw new errors.TypeError('ERR_INVALID_ARG_TYPE', 'format', ['esm', 'cjs', 'binary', 'builtin']);
      return { url, format: resolved.format };
    }
    if (!ModuleRequest.loaders.has(resolved.format) &&
        !(resolved.format === 'dynamic' && this.dynamicInstantiate)) {
      throw new errors.Error('ERR_UNKNOWN_MODULE_FORMAT', resolved.format);
    }
