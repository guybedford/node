...
const { extractWasmGlobalImports } = require('internal/modules/esm/wasm_utils');
...
translators.set('wasm', async function(url, source) {
  ...
  const wasmGlobalImports = wasmImports.some(({ kind }) => kind === 'global') ?
    extractWasmGlobalImports(source) : [];
  const importsList = ArrayPrototypeMap(wasmImports, ({ module }) => module);
  ...
  const { module } = createDynamicModule(importsList, exportsList, url, (reflect) => {
    for (const { moduleName, globalList } of wasmGlobalImports) {
      const wrappedModule = ObjectAssign({ __proto__: null }, reflect.imports[moduleName]);
      for (const { fieldName, descriptor } of globalList) {
        wrappedModule[fieldName] = new WebAssembly.Global(descriptor, wrappedModule[fieldName]);
      }
      reflect.imports[moduleName] = wrappedModule;
    }
    const { exports } = new WebAssembly.Instance(compiled, reflect.imports);
    for (const expt of exportsList) {
      const val = exports[expt];
      reflect.exports[expt].set(wasmGlobalExports.has(expt) ? val.value : val);
    }
  });
