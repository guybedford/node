...
PackageConfig emptyPackage = { false, false, "", false };
...
PackageConfig& GetPackageConfig(Environment* env, const std::string path) {
  ...
  PackageConfig pjson = { true, has_main, main_std, esm };
  return pjson_cache_[path] = pjson;
}

bool GetPackageModeEsm(Environment* env, const URL& search) {
  URL pjsonPath("package.json", &search);
  PackageConfig* pjson;
  do {
    pjson = &GetPackageConfig(env, pjsonPath.ToFilePath());
    if (pjson->exists) {
      break;
    }
...
void SetPackageModeEsm(Environment* env, const URL& search) {
  URL pjsonPath("package.json", &search);
  PackageConfig* pjson = &GetPackageConfig(env, pjsonPath.ToFilePath());
  pjson->exists = true;
  pjson->esm = true;
}
...
void ModuleWrap::Resolve(const FunctionCallbackInfo<Value>& args) {
  ...
  bool set_package_esm = args[2]->ToBoolean().As<v8::Boolean>()->Value();
