...
Maybe<const PackageConfig*> GetPackageConfig(Environment* env,
  ...
  Local<Value> mode_v;
  if (pkg_json->Get(env->context(), env->mode_string()).ToLocal(&mode_v)) {
    if (mode_v->StrictEquals(env->esm_string())) {
      esm = IsESM::Yes;
    }
  }

  auto entry = env->package_json_cache.emplace(path,
      PackageConfig { Exists::Yes, IsValid::Yes, has_main, main_std, esm });
  return Just(&entry.first->second);
}
